...
class SmartExitManager:
    ...
    def evaluate_exit(self, position: Any, market_data: dict[str, Any], indicators: dict[str, Any]) -> ExitDecision:
        ...
        structure_decision = self._evaluate_structure_break(side, highs, lows, closes)
        momentum_decision = self._evaluate_momentum_loss(side, highs, lows, closes, indicators)

        if structure_decision.confirmed and structure_decision.confidence >= (0.5 + req * 0.12):
            logger.info(
                "EXIT_DEBUG: symbol=%s exit_type=%s confirmed=%s confidence=%.2f reason=%s",
                symbol,
        ...
        logger.info(
            "EXIT_DEBUG: symbol=%s exit_type=none confirmed=False confidence=0.00 reason=%s",
            symbol,
            fallback_reason,
        )
    ...
    def _evaluate_structure_break(
        ...
        if side == "LONG":
            ...
            confirmed = close_break and follow_through
            ...
            reason = (
                ...
                f"structure_low={structure_low:.6f}"
            )
        ...
        reason = (
            ...
            f"structure_high={structure_high:.6f}"
        )
        return ExitDecision(confirmed, "structure", confidence, reason, confirmed)
    ...
    def evaluate_profit_protection(
        ...
        if pnl_r >= 1.0 and not bool(getattr(position, "breakeven_moved", False)):
        ...
        if pnl_r >= 2.0:
            ...
            if side == "LONG":
                trail_ref = min(lows[-3:]) if len(lows) >= 3 else current_price - risk * 0.6
                action.trailing_stop = max(entry, trail_ref)
            else:
                trail_ref = max(highs[-3:]) if len(highs) >= 3 else current_price + risk * 0.6
